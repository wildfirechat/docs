...
```
{
  ...
  "mediaType":2,
  "content":"{\"duration\":7}",
  "remoteMediaUrl":"https://media.wfcoss.cn/firechat/voice_message_sample.amr"
}
```
...
```
{
  ...
  "mediaType":1,
  "remoteMediaUrl":"https://media.wfcoss.cn/firechat/image_message_sample.jpg",
  "searchableContent":"[图片]",
...
```
{
  ...
  "remoteMediaUrl":"https://media.wfcoss.cn/firechat/video_message_sample.mov"}
```
...
```
生成无产出或者失败时发送。客户端收到后按流的ID删除对应的生成中/已生成消息，此消息自身不落库。
...
## 自定义消息
自定义消息可以把各种数据放到服务端的MessagePayload中，然后客户端来实现客户端的MessagePayload和自定义消息的转换。
