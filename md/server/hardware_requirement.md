...
## 内存
IM服务的内存参考，[内存选择](server_memory.md)。其它非核心服务可以使用很少的配置，需要自行评估。

## CPU
数据库和IM服务要分开部署，社区版服务每核心每秒能处理300条消息的分发，专业版性能更高，可以参考野火性能测试。可以评估业务的高峰分发量，评估需要的CPU情况。数据库需要支撑每秒的协议数。其它非核心服务可以使用很少的配置，需要自行评估。
