# 试用服务
试用服务仅针对需要付费的组件，免费组件可以直接商用。我们所有的收费产品，都提供试用服务，用户可以获得试用软件包或SDK，部署在自己的服务器上，用于试用体验，二次开发验证，给领导汇报或者给自己的客户演示。我们的产品做得好不好，相信您深度使用上几个月一定会心里有数的。
...
## 试用费用
试用费为300元，可以试用所有内容。试用后，如果购买，将抵扣300元的购买费用；如果不购买，将不予退还。试用费需要***公对公转账***（因为需要确认使用方身份），如果是国企或者政府单位因为流程不方便支付，可以联系我们使用其他方式验证身份和支付试用费用。
...
3. 微信添加 wfchat 或 wildfirechat， 联系我们，公对公发起转账支付300元试用费，包括所有非源码产品的试用，不是累加计费。
