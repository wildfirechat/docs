...
可是大家接触到的，都是下载下来直接就可以运行的，直接就可以聊天的应用，跟上面所说的矛盾啊。其实您所看到的应用都是我们提供的产品级开源客户端，包括各端应用（iOS/Android/PC/Web）及配套的应用服务，功能完善，可以直接商用，也可以在此基础上进行二次开发。即时通讯组件的应用可远远不止是做一个微信这一种可能。请参考我们的文章[野火IM的应用](https://zhuanlan.zhihu.com/p/60797772)。

那么那些是组件，那些是开源客户端应用呢？如下图：
...
图中黄色部分就是通讯组件嵌入的宿主应用系统。我们提供了一套完整的产品级开源通讯应用，包括应用服务（appserver）、各个端的客户端应用（iOS/Android/PC/Web）和机器人服务等。如果您有一套业务系统，可以通过接口跟即时通讯系统进行对接。如果您没有现成的业务系统，也可以直接使用我们的开源客户端系统。
...
5. 我现在有一套系统，我该如何把野火集成到我们的系统中去？
   ...
   把野火IM开源客户端中的逻辑迁移到您的系统中就可以了。其中在应用服务的逻辑就是换取token，在用户登录时去IM服务换取token返回给客户端，另外就是根据您的业务调用IM服务的接口给用户发消息。在客户端就是把野火IM的SDK集成进app，使用token调用connect，然后调用UI组件展现消息，也可以注册消息监听，监听业务消息内容。
