...
## 3. 野火IM如何处理
以野火IM 客户端为例，所有的网络访问分为应用和SDK两部分，以下分别说明

### 3.1 野火IM 客户端应用层的网络访问分析
野火IM 应用服务是个演示服务，正常情况下客户应该会使用自己的应用（需要把野火应用服务中获取token的功能迁移到客户自己的应用服务中），也可以直接使用。目前主要的功能只有登录，手机扫码/扫码确认登录等少数几个接口。客户端使用的都是系统提供的高级API，可以支持HTTPS。处理方法就是直接在服务器端支持HTTPS，然后客户端的APP Server Address换成HTTPS的地址即可。
```
