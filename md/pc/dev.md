# 二次开发
[vue-pc-chat](https://github.com/wildfirechat/vue-pc-chat)是一个产品级开源客户端，推荐参考```vue-pc-chat```进行二次开发
...
在PC客户端中，我们实现的是扫码登录的逻辑，可以重写这个逻辑（改成你们期望的方式，比如用户名/密码登录，或者OAuth登录等等），用户登录时去客户系统登录，只需要返回token给IM，然后再参考我们开源客户端的流程来处理就行。

当然应用服务中还有一些其它逻辑，比如群公告，收藏等小功能也都需要移植到客户系统中并对接调试。
