...
## 消息什么情况下会重复？
A：在发送方弱网的情况下，可能消息发送到IM服务后，IM服务回复ack失败，这样客户端会再尝试一次，如果成功就会产生发送一条，产生2条的情况。***一般情况下比较少见，错误不算严重，而且彻底解决的代价太大，所以就保持现状。*** 如果您的场景中某些消息严格不能出现重复，可以考虑在消息中加上业务唯一id，然后在收到后根据唯一id去重。可以在UI层存储这种关键消息的业务唯一id和messageid的对应关系（放到Share Preference中，或者数据库中），当收到新消息后就可以快速进行排重了。
