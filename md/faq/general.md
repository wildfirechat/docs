...
有时可能需要修改之后及时通知对方，解决办法有2中：1，发一条自定义透传消息，当用户收到后再发起强制更新。2，发一条不存储的消息，当用户收到后再发起强制更新。因为透传消息是不存储的，所以不会在服务器上留下痕迹，但有可能会导致用户不在线收不到而上线又没有刷新界面；普通不存储消息（是客户端不存储，但服务器端存储）会导致存储在服务器，但不会遗漏。判断的标准就是低频还是高频的操作，如果是低频操作就发不存储消息，如果是高频操作建议用透传消息。
...
A: 这些功能都不属于IM的功能，都是应用层的业务，这里只简单说一下。二维码实质是个URI，扫码只是方便手机输入这个URI，跟手动输入也没有实质性的区别。这个URI格式和意义需要展示方和扫码方共同支持，这样才能在双方直接传递信息。一个URI正常情况下是这种格式的 ***[scheme:][//authority][path][?query][#fragment]***。Scheme是确定是那个 应用提供服务的，移动端需要去注册这个Scheme，我们提供的开源客户端中使用的Scheme是***wildfirechat***。这样当移动端浏览器打开这个scheme的URI时会自动打开我们的应用，并把URI传递给我们。Authority是服务的类型，Scheme确定了是那个应用，Authority则是定义了这个应用中的那个服务。Path定义了服务的具体内容。野火开源客户端中使用了如下三个URI
```
...
野火开源客户端做了比较完善的优化，使用野火SDK进行开发的客户常常在这里遇到问题，收到大批量的消息导致大量刷新UI，导致大量请求用户信息和群组信息，导致客户端或者服务器端超频，导致部分数据同步不下来。使用野火SDK进行开发一定要注意，***只有在连接状态变为1时才能进行界面刷新，还有收到usersetting_updated事件也要刷新***。
...
## 野火为什么要限制只能使用80端口？什么情况下才会给开通定制端口？
