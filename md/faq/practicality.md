...
A。 首先我们不认同微信的模式，当部分人在群内进行通话时，所有人都可以点击加入，带来隐私性的问题。其次如果客户坚持需要此种模式，是可以根据SDK来自定义开发的，具体实现步骤如下。更详细的实现方案请参考[多人音视频的主动加入功能](../blogs/多人音视频的主动加入功能.md)：
...
> SpringBoot可以配置多个数据源，如果您使用野火提供的应用服务器，请给应用服务器配置上IM数据库数据源。请注意IM数据库一定要只读不能修改，如果需要修改请通过api进行修改，因为IM服务器使用了缓存，直接修改数据库容易导致数据不一致。
...
目前所有开源客户端都是这样实现的，在连接状态是接收中收到消息不处理，接收完成之后统一刷新。这样会有一个限制是，不能依赖接收到的消息来做业务处理，比如收到某个消息然后做什么事，特别是web端要避免依赖消息来驱动业务。
...
在微信和其他的一些主流社交软件中，会保留每个群成员的入群方式，包括邀请、分享名片、搜索等，会显示在入群提示和群成员用户详情。野火IM的开源客户端中没有实现这个功能，但这个功能也可以二开实现，以二维码扫码入群为例，我们的推荐如下：
...
## Q. 如何实现消息阅后即焚功能。
A. 可以利用野火IM服务的删除消息server api实现。最简单的实现方式是：
1. 客户端发送时，选择阅后即焚模式，在消息的extra中，添加对应的字段。
2. 在接收端收到消息后，发现是阅后即焚消息，展示信封封住，点击消息的信封请求阅读。
3. 点击后请求业务服务（请求中带有消息id），业务服务启动5秒销毁定时器。
4. 时间到之后，调用server api删除该消息，同步到两端就会删除。

可以更精细地加一些控制，比如：
1. 填写pushContent内容，这样推送时，就不会显示文本信息；
2. 在点击查看时，本地也开始计时，计时到期后也要本地调用删除；
3. 为了防止杀应用，本地及时写入到文件中，杀应用启动后

进一步提高安全性的做法就是，在发送时先请求业务服务，带上目标会话，获得一个id和密钥，用密钥加密内容，id明码保存。当点击查看时检查用户和会话是否对应，返回密钥并开始计时，客户端收到密钥解密打开。