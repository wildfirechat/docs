...
## Q.消息存储在数据库中的那张表中
A. 关于消息的表有两个，一个是消息表，这个表里存储着消息的原始数据，另外一张是用户消息表，记录用户的消息记录。使用h2db存储消息时，消息存储在t_message表中，用户消息记录在t_user_message表中；使用mysql存储消息时，消息存储在t_message_X表中，X等于 ***月份-1+(年份%3)\*12*** ，例如2019年4月存储在t_message_3表中(4-1+(2019%3)*12)。用户消息记录表是 t_user_message_Y，记录用户和消息的对应关系，分表规则是用户id hash的绝对值再对128去模，比如用户id为 ```userId1```，它的hash绝对值是 ```-147154261```，绝对值取模就是85，所以这个用户的消息记录在t_user_message_85表中。
